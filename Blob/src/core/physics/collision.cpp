...
void PhysicsEngine::Collision::ResolveWindow(Point &point,
                                             const sf::RenderWindow &window) {
  if (point.position.x - point.radius < 0 ||
      point.position.x + point.radius > window.getSize().x) {
    point.position.x = std::clamp(point.position.x, point.radius,
                                  window.getSize().x - point.radius);
    point.velocity.x = -point.velocity.x;
  }

  if (point.position.y - point.radius < 0 ||
      point.position.y + point.radius > window.getSize().y) {
    point.position.y = std::clamp(point.position.y, point.radius,
                                  window.getSize().y - point.radius);
...
void PhysicsEngine::Collision::ResolveCircle(Point &a, Point &b) {
  ...
  if (distance < totalRadius && distance != 0.0f) {
    ...
    float v1PrimeX = (v1x * (a.mass - b.mass) + 2 * b.mass * v2x) *
                     a.restitution / (a.mass + b.mass);
    float v2PrimeX = (v2x * (b.mass - a.mass) + 2 * a.mass * v1x) *
                     b.restitution / (a.mass + b.mass);
